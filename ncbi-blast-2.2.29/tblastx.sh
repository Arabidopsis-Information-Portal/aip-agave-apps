...
if [ -e $FILENAME ]; then
	...
	DBS="${database}"
	ARGS="${evalue} ${ungapped} ${max_target_seqs} ${filter} ${lowercase_masking} ${wordsize} ${matrix} -num_threads 2"
	
	# Not used by TBLASTX so we don't insert them above
	# gencode penalty reward gapopen gapextend
...
case ${format} in
	...
	${DOCKER_COMMAND} ${DOCKER_IMAGE} tblastx -db "${DBS}" ${ARGS} -query $FILENAME -out tblastx_out
